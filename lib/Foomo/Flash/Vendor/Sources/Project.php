...
namespace Foomo\Flash\Vendor\Sources;
...
class Project
{
	 ...
	 */
	public function __construct($id, $group, $version, $type, $name, $description, $exclude, $pathname)
	{
		$this->id = $id;
		$this->group = $group;
	...
	{
		return str_replace('.', '-', substr($this->id, strripos($this->group, '.') + 1)). '-' . $this->version;
	}
